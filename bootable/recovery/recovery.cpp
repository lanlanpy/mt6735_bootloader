...
#ifdef ZTE_FOTA
static const char *OTA_RESULT_TAG = "/cache/fota/error_code";      
#endif
...
main(int argc, char **argv) {
    ...
    if (update_package != NULL) {
		    ...
		    FILE *fp = fopen_path(OTA_RESULT_TAG,"w");
		    if (fp == NULL){
                LOGE("Can't open %s\n", OTA_RESULT_TAG);
		    }else{
	    	    fwrite(buf,4,1,fp);         // CLEAR the tag.
           	    check_and_fclose(fp, OTA_RESULT_TAG);
		    }
            #endif
            status = install_package(update_package, &wipe_cache, TEMPORARY_INSTALL_FILE, true);
            #ifdef ZTE_FOTA
		    if(status == INSTALL_SUCCESS)
    	    {
			    ...
			    if (fp == NULL){
	    		    ...
	    		    fwrite(buf,4,1,fp);       // A tag is added to indicate GOOGLE OTA upgrade success or not.
           		    check_and_fclose(fp, OTA_RESULT_TAG);
			    }
		   }
           #endif		
    ...
    if (update_package
		...
		// wuhuaiqin modify begin for do not remove update.zip file
        if (update_package) {
            {
                if(0==strcmp(update_package,"/sdcard/update.zip") || 0==strcmp(update_package,"/sdcard2/update.zip"))
                {
                    fprintf(stdout, "write result : sdcard update do not remove file\n");
                }
                else
                {
                    fprintf(stdout, "write result : remove_mota_file(%s)\n",update_package);
                    remove_mota_file(update_package);
                }
            }
        }
		// wuhuaiqin modify end
#ifdef SUPPORT_FOTA
