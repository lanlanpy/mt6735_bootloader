...
#define MMCBLOCK_BOOT0_READ_WRITE_FLAG "/sys/block/mmcblk0boot0/force_ro"
...
void mt_SetEmmcPreloaderWritable(State *state, const char *filename)
{
    ...
    if(fd < 0)
        uiPrintf(state, "Error: Open %s error", filename);
    ...
    if (!strcmp(partition, "logo") || !strcmp(partition, "preloader") || !strcmp(partition, "uboot") || !strcmp(partition, "dsp_bl") || !strcmp(partition, "bootimg") || !strcmp(partition, "boot") || !strcmp(partition, "tee1") || !strcmp(partition, "recovery")) {
        ...
        if (is_gpt) {
            ...
            if (fd != -1) {
                ...
                int len;
                int in_fd;
                off64_t preloader_pos;

                if (!strcmp(partition, PRELOADER_PART)) {
                  mt_SetEmmcPreloaderWritable(state, MMCBLOCK_BOOT0_READ_WRITE_FLAG);
                  preloader_pos = lseek64(fd, 0x800, SEEK_SET);
                  if(preloader_pos != 0x800){
                    uiPrintf(state, "%s line %d, failed to seek to preloader 0x800\n",__FUNCTION__,__LINE__);
                  }
                }
                    ...
                    if (sscanf(buf, "%s %s %s %d %s", p_name, p_size, p_addr, &p_type, p_actname) == 5) {
                        ...
                        if (strlen(p_name) && !strcmp(partition, p_name)) {
                            if (p_type == 2) {
                                ...
                                if(!strcmp(p_name, "preloader"))
                                  snprintf(part_name, 128, "/dev/block/mmcblk0boot0");
                                else
