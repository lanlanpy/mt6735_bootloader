...
#if 0
#define TTYDBG(fmt,args...) printf("[%s] %s %d: "fmt, __FILE__,__FUNCTION__,__LINE__,##args)
#else
#define TTYDBG(fmt,args...) do{}while(0)
#endif

#if 0
#define TTYERR(fmt,args...) printf("ERROR![%s] %s %d: "fmt, __FILE__,__FUNCTION__,__LINE__,##args)
#else
          ...
          while (buf->size > 0)
            {
                ...
                if (!current_urb
                  ...
                  {
                      printf ("write_buffer, no available urbs\n");
                      return;
                  }
...
//#define usbtty_event_log printf
          ...
          if (usb_input_buffer.size > 0)
            {
                ...
                //printf("usb_input_buffer.data = %s\n",usb_input_buffer.data);
