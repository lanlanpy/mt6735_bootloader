...
#define BATTERY_LOWVOL_POWEROFF_CHANGING_THRESOLD 3200
#endif
...
void check_bat_protect_status()
{
    ...
    while (bat_val < BATTERY_LOWVOL_POWEROFF_CHANGING_THRESOLD)
    {
        mtk_wdt_restart();
    ...
    dprintf(CRITICAL, "[%s]: check VBAT=%d mV with %d mV, stop charging... \n", __FUNCTION__, bat_val, BATTERY_LOWVOL_POWEROFF_CHANGING_THRESOLD);
}
